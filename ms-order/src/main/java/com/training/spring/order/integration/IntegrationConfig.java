...
public class IntegrationConfig {
    ...
    public static class MyRestClientErrorHandler implements ResponseErrorHandler {
        ...
        public void handleError(final ClientHttpResponse responseParam) throws IOException {
            if (IntegrationConfig.logger.isInfoEnabled()) {
                IntegrationConfig.logger.info("[ClientConfig.MyRestClientErrorHandler][handleError]-> Error Status code : "
    ...
    public static class MyRestClientInterceptor implements ClientHttpRequestInterceptor {
        ...
        public ClientHttpResponse intercept(final HttpRequest requestParam,
            ...
            ClientHttpResponse executeLoc = executionParam.execute(requestParam,
                                                                   bodyParam);
            if (IntegrationConfig.logger.isInfoEnabled()) {
                IntegrationConfig.logger.info("[ClientConfig.MyRestClientInterceptor][intercept]-> Call Delta : "
    ...
    public ConnectionKeepAliveStrategy connectionKeepAliveStrategy() {
        return IntegrationConfig::getKeepAliveDurationProxy;
    }
    ...
    public static long getKeepAliveDurationProxy(final HttpResponse r,
        ...
        try {
            ...
            IntegrationConfig.logger.error("[ClientConfig][connectionKeepAliveStrategy]-> *Error* : "
                                           + eLoc.getMessage(),
                                           eLoc);
        }
        return 60_000;
